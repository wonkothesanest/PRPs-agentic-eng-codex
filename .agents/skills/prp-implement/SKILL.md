---
name: prp-implement
description: Execute an implementation plan with rigorous validation loops. Use when a developer has a plan file and wants to execute it. Accepts a path to a .plan.md file.
---

# Implement Plan

**Plan**: The plan file path from the developer's request
...
Read the plan file at the path specified by the developer. If the path was not provided, look for the most recent plan in `.claude/PRPs/plans/`.
...
```
Error: Plan not found at {plan-file-path}
...
mv {plan-file-path} .claude/PRPs/plans/completed/
```
